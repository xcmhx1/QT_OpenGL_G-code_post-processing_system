...
- `ProcessOverride`：保存加工启用、方向、起点、手动内部线三态、人工顺序、加工断面角色和断面组编号。
...
加工启用、方向、起点、人工顺序、Break/Waste 角色和内部线状态均按图元身份保存。
...
- 设置、清除单个或批量清除人工顺序均按上述 revision 规则处理。
...
- 人工加工顺序以 `std::optional<int>` 保存，本阶段不要求唯一，也不自动重排编号。
...
|---|---|---|---|
| 人工加工顺序 | 已有正式状态、查询和规划输入字段 | 普通排序应优先使用人工顺序 | 计划构建器本阶段尚未消费该字段 |
