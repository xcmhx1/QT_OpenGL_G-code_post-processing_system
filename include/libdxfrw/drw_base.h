...
//! 用于处理 DXF 头部变量（Header Vars）的变体类
/*!
 *  该类实现了一个通用的数据容器，可以根据需要存储字符串、整数、浮点数或坐标点。
 *  它是解析 DXF 文件中 HEADER 部分系统变量的核心数据结构。
 *  @作者 Rallaz
 */
class DRW_Variant {
public:
    ...
    // 拷贝构造函数
    // 注意：当类型为 COORD 或 STRING 时，必须将 content 联合体内部的指针重新指向当前对象的成员变量地址
    DRW_Variant(const DRW_Variant& d) : sdata(d.sdata), vdata(d.vdata), content(d.content), vType(d.vType), vCode(d.vCode) {
        if (d.vType == COORD)
            content.v = &vdata; // 修正指针，指向本对象的坐标存储区
        if (d.vType == STRING)
            content.s = &sdata; // 修正指针，指向本对象的字符串存储区
    }
    ...
    // 单独设置坐标的 X/Y/Z 分量（仅在当前类型为 COORD 时有效）
    ...
    // 返回该变量对应的 DXF 组码（Group Code）
    int code() { return vCode; }

public:
    std::string sdata;    // 字符串数据的实际存储空间
    DRW_Coord vdata;      // 坐标数据的实际存储空间

private:
    // 内部联合体：用于在相同的内存位置存储不同类型的值，以节省空间
    union DRW_VarContent {
        UTF8STRING* s;    // 指向字符串存储区的指针
        dint32 i;         // 直接存储整数值
        double d;         // 直接存储双精度浮点数值
        DRW_Coord* v;     // 指向坐标存储区的指针
...
public:
    DRW_VarContent content; // 变体的实际内容（通过此联合体访问）

private:
    enum TYPE vType;        // 记录当前变体存储的具体类型
    int vCode;              // 记录该变量在 DXF 中的组码
};
