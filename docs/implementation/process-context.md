...
当前生产代码将加工启用、方向、起点、Break/Waste 角色和内部线状态按图元身份保存。
...
- 复制图元不会复制自动内部线结果，新图元在后续排序中形成新的加工单元身份。
...
- `ProcessOverride::manualProcessOrder` 及其设置、查询和规划输入已经移除。
...
| 连续关系变化 | 删除成员时移除对应旧单元，重新排序时生成新单元身份 | 连续关系变化后重新解析单元序列 | 当前普通排序尚未利用旧序列保留可匹配单元的相对顺序 |
