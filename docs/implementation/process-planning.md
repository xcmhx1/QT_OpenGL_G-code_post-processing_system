...
- 四轴规划直接使用当前有效截面参数，不按文档 revision 判定截面过期；解算失败时返回具体错误提示用户检查截面。
...
- 手动内部线标记解析为最终布尔值，规划器只接收解析后的结果；内部线自动清理直接删除窗口内图元，不产生自动排除状态。
- 四轴普通加工单元的连通分量只在最终可加工实体子集内建立；已排除的内部线和 Waste 图元不参与普通单元的邻接传播。
...
- 三轴 NC 对多成员加工单元按连续切削输出：单元首成员执行完整安全转移与接近，后续成员不再输出起始快速定位，切削直接接续上一成员终点；某成员无法生成 NC 运动时，其后的成员边界恢复安全转移。三轴仍按成员输出图层、颜色和类型规则块，不引入单元级启停代码。
...
- 入口区位由入口点和其后的第一个非退化切削段共同确定。首刀内部采样稳定落在同一分界母线时，该母线可以形成合法入口；仅端点接触母线、跨越相邻区位或投影不稳定的短段仍不能形成入口。合法候选在方管展开空间中比较入口反向、切线连续性、X 命中距离、旋转、移动和稳定身份。
...
- 四轴计划发布前使用 `ProcessUnitExecutionResolver` 校验每个单元的源空间连接、机床 XYZ 连接和连续 A 轴。开放单元在某个相邻成员边界失败时拆为两个原子单元并重新建立连续编号；闭环、片段单元或无法安全拆分的异常继续作为计划失败。
- 普通排序恢复当前单元序列和人工遍历覆盖后再次执行同一连续性校验，避免覆盖后的最终执行方向绕过计划发布检查。
