...
- 提供仿 AutoCAD 风格的紧凑工具面板，当前分为 `默认` 与 `机加工` 两个页签；`默认` 页签包含“绘图”“修改”“图层”“特性”四个左对齐面板，`机加工` 页签已收束为“导入导出”“排序”“功能”“配置”“G代码配置”五个面板
...
- 提供 `用户设置 -> G代码配置...` 对话框，并已纳入当前浅色 / 深色主题；当前对话框已补充 `四轴加工` 配置页，可设置四轴离轴额外距离、加工面 `Z` 修正等参数，并已拆分“恢复3轴默认 / 恢复4轴默认”
...
7. 四轴模式下，图元间空跑采用“导出开始时一次性解算的全图最大离 `X` 轴距离 + 四轴离轴额外距离”作为统一安全高度，默认额外距离为 `5`
8. 四轴模式下，安全路径的抬升、横移、转 `A`、下落全部使用 `G00`；必要时会在图元边界插入显式安全过渡动作
9. 若当前 Profile 启用了四轴初始机床点，生成器会在进入首个待加工图元前先输出这段初始定位
10. 导出时会按“文件级 -> 图层规则 -> 颜色规则 -> 实体类型”的顺序组合头尾代码块
11. 对 `Circle` 默认以顶部为起刀点；对完整 `Ellipse` 与闭合 `Polyline/LWPolyline`，会按排序阶段确定的缝点导出
12. 导出时会跳过空白代码块中的无意义空行，减少对机床无效的文本噪声，并统一使用 `CRLF` 行尾
13. 生成器在导出时弹出文件保存对话框并输出 `.nc/.gcode/.txt`
