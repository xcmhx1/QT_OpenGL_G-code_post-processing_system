...
- `3D` 导出的旋转轴参数当前仅部分暴露在 `GProfile` 数据模型与 JSON 读写中，配置对话框暂未提供完整四轴工艺编辑界面
