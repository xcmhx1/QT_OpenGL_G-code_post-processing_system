...
- 入口精化以实际前序切削终点按共享安全距离规则生成的 `previousRetractPose` 为接近起点，并在方管展开空间中比较接近距离和方向。
...
- 当前 owner 区位没有通过全部校验的真实曲线切点时，先选择距离 `previousRetractPose` 最近的合法成员连接点；距离在并列容差内时稳定消歧，同一连接点选择与接近方向夹角更小的闭环方向。连接点也不存在时，才在最长可靠连续区位段上取三维弧长中点。
...
- 单图元圆、完整椭圆和闭合样条继续沿用其独立入口规则；本次真实曲线切点仅作用于智能重建、懒旋转、有效截面下的普通多图元闭环。
