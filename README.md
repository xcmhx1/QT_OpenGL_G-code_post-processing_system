...
本项目是一个基于 Qt 6 Widgets、OpenGL 4.5 Core Profile、Visual Studio 2026 的 Windows 桌面 CAD / G-code 后处理程序。当前代码主线已经具备 CAD 文件导入、位图矢量化导入、二维图元显示、基础交互、简单绘图与编辑命令，并提供 `2D` G 代码生成后端与 JSON Profile 配置能力；主窗口已接通导入图片、导出 G 代码、反向加工以及按 `2D / 3D` 分类的排序菜单，其中 `2D` 下提供“排序（保留方向）”与“智能排序”，`3D` 排序逻辑保留，`3D` G 代码生成链路已整体清理并停用，等待按新的工艺模型重写。同时增加了仿 AutoCAD 风格的紧凑 Ribbon 工具面板，用于统一承载绘图、修改、图层与特性入口；其中“修改”面板现已接通 `移动`、`删除`、`旋转`、`复制`、`缩放`、`矩形阵列` 六类图元编辑操作，并提供显式的浅色 / 深色主题切换。当前“用户设置”菜单下也已接通 `G代码配置` 对话框，可按文件级、实体类型、图层规则、颜色规则四个层级定制导出行为，其中颜色规则已切换为以 AutoCAD 基础索引颜色为主的配置模型；Viewer 层也已支持加工方向箭头、加工顺序编号、选中图元基点/控制点手柄显示，底部状态栏也已接通基点 / 控制点 / 网格三类吸附开关，并已接入第一阶段控制点编辑（选中后点控点再点目标点）。
...
- 网格显示已区分主/次网格线：主网格线保持标准网格色，次网格线按背景色做淡化处理，降低高密度缩放下的视觉噪声
...
- 提供第一阶段控制点编辑：在空闲态点击“当前选中图元”的可编辑控制点后，再次点击目标点提交；当前支持 `Line`、`Polyline`、`LWPolyline`
- 提供 transient 预览、十字光标叠加、选中高亮
- 选中图元时已显示基点/控制点手柄；圆弧与椭圆边界端点使用三角箭头手柄强调方向端
...
- 已接通主窗口“导入文件”“导入图片”“导出G代码”“反向加工”以及 `排序 -> 2D / 3D` 分类菜单动作
...
- 控制点编辑当前为第一阶段实现：采用“两次点击提交”交互，不包含拖拽实时预览；仅 `Line`、`Polyline`、`LWPolyline` 的顶点为可编辑控制点
...
- 文件 -> `导入图片`：直接进入位图导入对话框
- 文件 -> `导出G代码`：导出当前文档的 G 代码；当前仅 `2D` 可用，`3D` 模式会直接提示待重写
...
16. 选中点、线、圆、圆弧、椭圆、多段线、轻量多段线，验证 Viewer 中基点/控制点手柄显示是否正确
...
- 控制点编辑当前仅覆盖 `Line`、`Polyline`、`LWPolyline`，其余图元手柄暂不支持直接编辑
