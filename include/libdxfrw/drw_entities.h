...
*  @author Rallaz
*/
//所有CAD图形实体的基类，定义通用属性和解析接口
//该类不直接实例化，需通过派生类实现具体实体类型
// 所有实体类的基类
class DRW_Entity {
    // 声明宏，允许DXF/DWG读写类访问受保护成员
    SETENTFRIENDS
    //friend class dxfRW;
    //friend class dwgReader;
public:
    ...
    // 重置实体状态，清空扩展数据和当前解析状态
	void reset() {
		extData.clear();// 清空扩展数据列表
		curr.reset();   // 重置当前解析缓冲区
	}
...
protected:
    ...
    //解析DXF组码（组码对应实体属性，如层、颜色等）
    virtual bool parseCode(
        int code,                                   // @param code DXF组码（如0-实体类型，8-图层名）
        const std::unique_ptr<dxfReader>& reader    // @param reader DXF读取器实例
    );

    //calculates extrusion axis (normal vector)
    // 计算实体拉伸轴（法向量）
    void calculateAxis(
        DRW_Coord extPoint                          // @param extPoint 拉伸基点坐标
    );

    //apply extrusion to @extPoint and return data in @point
    // 应用拉伸变换到指定点
    void extrudePoint(
        DRW_Coord extPoint, //拉伸基点
        DRW_Coord *point    //输出参数，存储变换后的坐标
    );
    ...
    virtual bool parseDwg(
        DRW::Version version,   // 文件版本
        dwgBuffer *buf,         // 文件数据缓冲区
        dwgBuffer* strBuf,      // 字符串缓冲区
        duint32 bs=0            // 起始位置
    );
    ...
    // 解析DXF组码102的扩展实体数据（XDATA）
    // 专门处理实体扩展数据块，格式为：102 { ... } 或 102 "APPNAME" ... 
    // 参数code用于标识当前组码是否为102
    bool parseDxfGroups(
        int code,                                   // 组码
        const std::unique_ptr<dxfReader>& reader    // dxf读取器实例
    );
...
public:
    ...
    // 扩展数据列表（存储F code 1000-1071的扩展实体数据）
