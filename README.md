...
- 提供仿 AutoCAD 风格的紧凑工具面板，当前分为 `默认` 与 `机加工` 两个页签；`默认` 页签包含“绘图”“修改”“图层”“特性”四个左对齐面板，`机加工` 页签包含导入导出、排序与 G 代码配置快捷入口
...
- 提供 `GProfile` JSON 配置读写，支持文件级、实体类型、图层规则、颜色规则四类配置，并已新增四轴旋转轴配置字段
- 提供 `用户设置 -> G代码配置...` 对话框，并已纳入当前浅色 / 深色主题；当前对话框已补充 `四轴加工` 配置页，可设置四轴离轴额外距离等参数
...
- 当前 `Line`、`Circle`、`Ellipse` 已完成较稳定的四轴控制点生成；`Arc`、`Polyline`、`LWPolyline`、`Point` 也已接入统一接口，但工艺行为仍需继续验证与收敛
- 四轴导出当前会在图元边界对齐 `A` 角连续性，避免如 `180 -> -90` 这类跨象限切换走优弧空跑
- 四轴图元间空跑当前采用“全图最大离 `X` 轴距离 + 50`(5cm)`”作为统一安全高度，安全高度的抬升、横移、转 `A`、下落全部使用 `G00`
...
- 当前四轴导出链只对绕 `X` 轴回转的工件场景建模，不是通用多轴后处理器
- `Line`、`Circle`、`Ellipse` 的四轴控制点生成已基本可用；`Arc`、`Polyline`、`LWPolyline`、`Point` 仍需继续补齐工艺细节与充分验证
- `3D` 排序相关逻辑仍以历史实现为主，尚未围绕当前四轴控制点链路整体重写
- `3D` 导出的旋转轴参数当前已开始接入 `G代码配置 -> 四轴加工` 配置页，但整体仍以基础参数为主，后续还需要继续补齐更完整的四轴工艺编辑界面
...
- 图元间空跑使用统一安全高度：
  - 全图所有图元到 `X` 轴的最大距离
  - 再额外加 `50`（按当前项目单位，相当于 `5cm`）
- 安全路径中的抬升、横移、旋转 `A`、下落全部走 `G00`
- 从安全高度回到加工起点时，不把这段路径当加工轨迹处理

### 当前已知限制

- 当前四轴链路仍然优先服务回转类、截面类工件，不适合直接理解为任意自由曲面的通用后处理
- `Arc`、`Polyline`、`LWPolyline`、`Point` 的机床控制点生成逻辑虽然已接入，但还没有像 `Line`、`Circle`、`Ellipse` 一样充分验证
...
- `机加工` 页签提供 `文件导入`、`G代码导出`、`排序(保留方向)`、`智能排序`、`G代码模式`、`G代码配置` 等高频入口
...
6. 四轴模式下，`GGenerator` 会按照 `CadItem::m_processOrder` 组织图元，并在图元边界对齐 `A` 角连续性，避免例如 `180 -> -90` 这类优弧空跑
7. 四轴模式下，图元间空跑采用“全图最大离 `X` 轴距离 + 50`(5cm)`”作为统一安全高度，安全路径的抬升、横移、转 `A`、下落全部使用 `G00`
...
- 当前验证较充分的图元是：
  ...
  - `Circle`
  - `Ellipse`
- `Arc`、`Polyline`、`LWPolyline`、`Point` 已接入统一接口，但仍应按实际工艺继续验证
...
- `默认`：保留 CAD 编辑主工作流，左对齐排列 `绘图`、`修改`、`图层`、`特性` 四个面板
...
- 四轴离轴额外距离：用于在一次导出开始时，以“全图最大离 `X` 轴半径 + 额外距离”解算统一安全高度；默认值为 `50`
...
- `Mode3D` 当前已切换到新的四轴导出主链，但适配重点仍集中在绕 `X` 轴回转的截面类工件
...
- 当前四轴导出的工艺语义和边界策略主要围绕 `Line`、`Circle`、`Ellipse` 收敛，其它图元不要默认视为已完全稳定
