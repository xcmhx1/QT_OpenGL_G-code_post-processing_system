...
- 相邻且独立成行的 `M05`/`M03` 或 `M5`/`M3` 会成对删除；中间存在其他代码时保留。
...
| 自动补计划 | 导出缺少有效计划时调用当前“智能排序”入口 | 智能排序应忽略人工顺序和方向 | 当前智能入口与普通排序共用算法并保留方向偏好 |
